...
class FaceProcessingThread2(QThread):
    ...
    def run(self):
        ...
        if args.network == 'yolo3_darknet53_voc':
            net.load_parameters('MaxWeight.params', ctx=ctx)
            print('use MaxWeight to extract feature')
        elif args.network == 'yolo3_mobilenet1.0_voc':
            net.load_parameters('MidWeight.params', ctx=ctx)
            print('use MidWeight to extract feature')
        else:
            net.load_parameters('MinWeight.params', ctx=ctx)
            print('use MinWeight to extract feature')
    ...
    def run(self):
        ...
        while self.isRunning:
            if CoreUI.cap.isOpened():
                ...
                # n = 1
                # timeF = 10
                # vc = cv2.VideoCapture("/Users/mac/Desktop/1.mp4")
                # rval, frame = vc.read()
                    ...
                    for (_x, _y, _w, _h) in faces:
                        ...
                        if self.isFaceRecognizerEnabled:
                            ...
                            if confidence < self.confidenceThreshold:
                                ...
                                today_will_insert_date = datetime.now().strftime('%Y-%m-%d')
                                out_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                try:
                                    if int(datetime.now().strftime('%H%M%S')) < 70000 and list(cursor.execute(
                                            "select count(*) from workers where in_time like '{0}%' and cn_name = '{1}'".
                                                    format(today_will_insert_date, en_name)))[0][0] == 0:
                                        cursor.execute('INSERT INTO workers (name,in_time) VALUES (?, ? )',
                                                       (en_name, in_time))
                                    if int(datetime.now().strftime('%H%M%S')) > 170000 and list(cursor.execute(
                                            "select count(*) from workers where in_time like '{0}%' and cn_name = '{1}'".
                                                    format(today_will_insert_date, en_name)))[0][0] == 1:
                                        cursor.execute(
                                            "update workers set out_time = '{0}' where cn_name = '{1}'".format(out_time,
                                                                                                            en_name))
