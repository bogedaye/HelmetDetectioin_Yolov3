    ...
    def migrateToDb(self):
        if self.isFaceDataReady:
            ...
            try:
                ...
                if cursor.fetchall():
                    text = '数据库已存在工号为 <font color=blue>{}</font> 的用户记录。'.format(stu_id)
                    ...
                    if ret == QMessageBox.Yes:
                        # 更新已有记录
                        cursor.execute('UPDATE users SET cn_name=?, en_name=? WHERE worker_id=?',
                                       (cn_name, en_name, stu_id,))
                    else:
                        raise OperationCancel  # 记录取消覆盖操作
                else:
                    # 插入新记录
                    cursor.execute('INSERT INTO users (worker_id, cn_name, en_name) VALUES (?, ?, ?)',
