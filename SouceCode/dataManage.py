...
class DataManageUI(QWidget):
    ...
    def queryUser(self):
        ...
        try:
            cursor.execute('SELECT * FROM users WHERE worker_id=?', (stu_id,))
            ret = cursor.fetchall()
            if not ret:
    ...
    def deleteUser(self):
        ...
        if ret == QMessageBox.Yes:
            ...
            try:
                cursor.execute('DELETE FROM users WHERE worker_id=?', (stu_id,))
            except Exception as e:
                cursor.close()
                ...
                text = '你已成功删除工号为 <font color=blue>{}</font> 的用户记录。'.format(stu_id)
    ...
    def prepareTrainingData(self, data_folder_path):
        ...
        for dir_name in dirs:
            ...
            try:
                ...
                if not ret:
                    raise RecordNotFound
                cursor.execute('UPDATE users SET face_id=? WHERE worker_id=?', (face_id, stu_id,))
            except RecordNotFound:
                logging.warning('数据库中找不到学号为{}的用户记录'.format(stu_id))
                self.logQueue.put('发现工号为{}的人脸数据，但数据库中找不到相应记录，已忽略'.format(stu_id))
                continue
